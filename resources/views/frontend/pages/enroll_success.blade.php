...
@section('contents')
    ...
    @include('frontend.partials.breadcrumb', ['title' => 'Application Received'])
            ...
            <div class="row justify-content-center">
                <div class="col-lg-7">
                    <div class="p-40 bg-main-25 rounded-16 border border-neutral-30 text-center">
                        ...
                        <p class="text-neutral-500 mb-32">Your admission application for <strong>{{ $enrollment->training->title }}</strong> has been received and is currently <strong>{{ $enrollment->status === 'paid' ? 'Approved' : ($enrollment->status === 'failed' ? 'Rejected' : ucfirst($enrollment->status)) }}</strong>. We will contact you shortly at {{ $enrollment->phone }}.</p>
                        ...
                        <div class="p-24 bg-white rounded-12 border border-neutral-30 text-start mb-32">
                            ...
                            <div class="flex-between gap-8 mb-12">
                                <span class="text-neutral-500">Candidate</span>
                                <span class="fw-medium text-neutral-700">{{ $enrollment->name }}</span>
                            </div>
                            <div class="flex-between gap-8 mb-12">
                                <span class="text-neutral-500">Admit Roll</span>
                                <span class="fw-medium text-neutral-700">{{ $enrollment->admit_roll ?? '-' }}</span>
                            </div>
                            <div class="flex-between gap-8">
                                <span class="text-neutral-500">Registration Fee</span>
