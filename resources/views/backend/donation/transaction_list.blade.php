@extends('backend.admin.master')

@section('admin_title', $title)

@section('admin_content')

    <div class="main-content">

        <section class="section">

            <div class="section-body">

                <div class="row">

                    <div class="col-12">

                        <div class="card">

                            <div class="card-header d-flex justify-content-between">
                                <h4>{{ $title }}</h4>
                                <h4>
                                    <a href="{{ route('admin.donation-category.list') }}" class="btn btn-outline-dark"><i class="fas fa-list"></i> Donation Categories</a>
                                </h4>
                            </div>

                            <div class="card-body">

                                @include('widgets.errors')
                                @include('widgets.success')

                                <div class="table-responsive">

                                    <table class="table table-striped table-hover" id="tableExport" style="width:100%;">

                                        <thead>
                                            <tr>
                                                <th>Invoice</th>
                                                <th>Donor</th>
                                                <th>Category</th>
                                                <th>Amount</th>
                                                <th>bKash Trx ID</th>
                                                <th>Status</th>
                                                <th>Date</th>
                                                <th>Action</th>
                                            </tr>
                                        </thead>

                                        <tbody>
                                            @foreach ($donations as $item)
                                                <tr>
                                                    <td>{{ $item->invoice_no }}</td>
                                                    <td>{{ $item->is_anonymous ? 'Anonymous' : $item->donor_name }}<br><small class="text-muted">{{ $item->donor_phone }}</small></td>
                                                    <td>{{ $item->category->title ?? 'General' }}</td>
                                                    <td>৳{{ number_format($item->amount, 2) }}</td>
                                                    <td>{{ $item->bkash_trx_id ?? '—' }}</td>
                                                    <td>
                                                        <div class="badges">
                                                            @if ($item->status == 'completed')
                                                                <span class="badge badge-success">Completed</span>
                                                            @elseif ($item->status == 'pending')
                                                                <span class="badge badge-warning">Pending</span>
                                                            @elseif ($item->status == 'cancelled')
                                                                <span class="badge badge-secondary">Cancelled</span>
                                                            @else
                                                                <span class="badge badge-danger">Failed</span>
                                                            @endif
                                                        </div>
                                                    </td>
                                                    <td>{{ $item->created_at->format('d M Y, h:i A') }}</td>
                                                    <td>
                                                        <div class="table_actions d-flex gap-2">
                                                            <a href="{{ route('admin.donation.show', $item->id) }}" class="btn btn-outline-primary" title="View"><i class="far fa-eye"></i></a>
                                                            <a href="#!" class="btn btn-outline-danger" data-del="{{ route('admin.donation.delete', $item->id) }}" data-bs-toggle="modal" data-bs-target="#donation_delete_modal" data-name="{{ $item->invoice_no }}" title="Delete"><i class="far fa-trash-alt"></i></a>
                                                        </div>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>

                                    </table>

                                </div>

                            </div>

                        </div>

                    </div>

                </div>

            </div>

        </section>

    </div>

    {{-- Delete Modal --}}
    <div class="modal fade" id="donation_delete_modal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title fs-5"></h3>
                </div>
                <div class="modal-body">
                    <h5>Are you sure you want to delete this donation record?</h5>
                </div>
                <div class="modal-footer" style="justify-content: space-between">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Close</button>
                    <a href="" id="final_delete" class="btn btn-outline-danger">Confirm Delete</a>
                </div>
            </div>
        </div>
    </div>
    {{-- End: Delete Modal --}}

@endsection

@section('footer_script')
    <script>
        $('#donation_delete_modal').on('show.bs.modal', function(event) {
            var button = $(event.relatedTarget)
            var name = button.data('name')
            var modal = $(this)
            modal.find('.modal-title').html('Delete <span class="text-danger">' + name + '</span>');
            $('#final_delete').attr('href', button.attr('data-del'))
        })
    </script>
@endsection
